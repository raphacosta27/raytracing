...
public:
  ...
  __device__ hitable_list(hitable **list, int size) : _list(list), _size(size) { }
  __device__ virtual bool hit(const ray& r, float t_min, float t_max, hit_record &record) const;
};

__device__ bool hitable_list::hit(const ray &r, float t_min, float t_max, hit_record &record) const {
